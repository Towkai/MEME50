...
ssize_t uart_read (struct file * filp, char *buf, size_t count, loff_t * t)
{
    ...
    while ( (rcount < count) && (inb(PORT_COM1+REG_LSR) & DATA_READY) ) {
      // To do: read recive character form register

    }
    if(rcount != 0 ) {
      printk(KERN_ALERT " uart_read(): %s\n",prbuf);
      // To do: copy the data to user space
...
ssize_t uart_write (struct file * filp, const char *buf, size_t count, loff_t * t)
{
    ...
    // To do: copy the data from user space
    

    while( i < count ) {
      if( inb(PORT_COM1+REG_LSR) & THRE_EMPTY ) {
        // To do: write to IO port, PORT_COM1
      }
    }
...
int __init init_module(void)
{
    ...
    // To do: allocate read/write buffer


    // To do: PORT_COM1, 9600, BITS_8 | PARITY_NONE | STOP_ONE
