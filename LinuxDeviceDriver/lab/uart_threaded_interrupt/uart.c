...
ssize_t uart_read (struct file * filp, char *buf, size_t count, loff_t * t)
{
    ...
    while ( (rcount < count) && (inb(PORT_COM1+REG_LSR) & DATA_READY) ) {
      // read recive character form register
      *(prbuf+rcount)=inb(PORT_COM1);
      rcount++;
    }
    if(rcount != 0 ) {
      printk(KERN_ALERT " uart_read(): %s\n",prbuf);
...
#endif
{
  ...
  if( reg & IIR_RECEIVED_AVAILABLE ) {
    ...
    if ( in_interrupt() )
      ...
      printk("Not in interrupt context\n");
  }
  else if ( reg & 0x02 ) {
    printk("uart_interrupt(): iir:%x, tx interrupt\n", reg);
  }
...
static irqreturn_t uart_thread_interrupt(int irq, void *dev_id) {

   printk("uart_thread_interrupt() is ");
   if ( in_interrupt() )
      printk("in interrupt context\n");
    else
      printk("not in interrupt context\n");
...
int __init init_module(void)
{
    ...
    // request and enable interrupt for COM1 and enable interrupt
#ifdef REQIEST_IRQ
    #if LINUX_VERSION_CODE < KERNEL_VERSION(2,6,24)
    ...
    #endif
#else
    // To do: Use the request_threaded_irq() to request interrupta

#endif

    outb(COM1_IRQ_ON, PORT_COM1+REG_MCR);
    outb(REC_IRQ_ON|0x02, PORT_COM1+REG_IER);
