...
/*  Create and name a socket for the server.  */
    ...
    // server_address.sin_addr.s_addr = inet_addr("127.0.0.1"); //使用./client2連線
    server_address.sin_addr.s_addr = htonl(INADDR_ANY); //使用telne連線
    // server_address.sin_port = 9734; //htons(9734);
    server_address.sin_port = htons(9734); //保證位元組順序正確，server/client其中一個要做
...
/*  If the activity is on server_sockfd, it must be a request for a new connection
    and we add the associated client_sockfd to the descriptor set.  */
                ...
                if(fd == server_sockfd) {
                    ...
                    // printf("clientfd = %s", ch);
                    // sprintf(ch, "%d", client_sockfd);
                    // write(server_sockfd, ch, BUFFER_SIZE);
                }
    ...
    Otherwise, we 'serve' the client as in the previous examples.  */
                    ...
                    if(nread == 0) {
                        ...
                        for (int i = 4; i < FD_SETSIZE; i++)
                            write(i, &ch, BUFFER_SIZE);
                        // write(client_sockfd, &ch, BUFFER_SIZE);
                    }
                }
            }
        }
